class Member < ApplicationRecord
  ...
  def update_address(new_address)
    ...
    if canonical_address = CanonicalAddress.search(address_attributes)
      if address = addresses.where(canonical_address: canonical_address).first
        address.touch
      else
        addresses << Address.new(address_attributes.merge(canonical_address: canonical_address))
      end
    else
      # If we can't match the address
      if address = addresses.where(address_attributes).first
        address.touch
      else
    ...
    unless self.address.try(:id) == old_address_id
      UpdateMemberAreasWorker.perform_async(id)
      return true
    end
